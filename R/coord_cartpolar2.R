...
#' @param grill draw the lines and labels of the polar grill in the foreground or
#'   background
#'
#' @export
#'
...
coord_cartpolar2 <- function(limit = NULL, clip = "on", bg = NULL, grill = c("background", "foreground")) {
  grill <- match.arg(grill)
...
CoordCartPolar2 <- ggproto("CoordCartPolar2", CoordCartesian,
  ...
  setup_panel_params = function(self, scale_x, scale_y, params) {
    ...
    if (scale_x$is_discrete()) {
      stop("coord_cartpolar2 doesn't support discrete x scale")
    }
  ...
  render_bg = function(self, panel_params, theme) {
    ...
    if (self$grill == "background") {
      elements <- c(elements, render_polargrill(self, panel_params, theme))
    }
...
render_polargrill <- function(self, panel_params, theme) {
  elements = list()
