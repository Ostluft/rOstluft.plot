...
#' )
#'
#' p <-  ggplot(df, aes(x=u, y=v)) + geom_point() + coord_cartpolar2()
#' p
#'
...
#'   coord_cartpolar2(labels_wd = element_text(color = "violet", size = 14, face = "bold")) +
#'   theme(
...
#' p + scale_x_continuous(limits = c(-10, 10), breaks = c(1, 2.5, 5, 10)) +
#'   theme("panel.grid.minor.x" = element_blank())
#'
#' # wd scale in coord_cartpolar constructor as arugment scale_wd
#' ggplot(df, aes(x=u, y=v)) + geom_point() +
#'   coord_cartpolar2(scale_wd = scale_wd_identity(
...
#' pbg <- ggplot(df, aes(x=u, y=v)) + geom_point() + coord_cartpolar2(bg = bg)
#' pbg
#'
#' # faceting
...
#' df <- expand.grid(u = -3:3, v = -3:3)
#' df$z <- runif(nrow(df))
#' ggplot(df, aes(x=u, y=v, fill=z)) +
#'   coord_cartpolar2(expand = FALSE, limit = 3.5) +
#'   annotation_raster(bg, -Inf, Inf, -Inf, Inf) +
#'   geom_raster(alpha = 0.5)

coord_cartpolar2 <- function(limit = NULL, expand = TRUE, clip = "on", bg = NULL, scale_wd = scale_wd_identity(),
                            labels_wd = element_text()) {
  ggproto(NULL, CoordCartPolar2,
    limit = limit,
    expand = expand,
    clip = clip,
    bg = bg,
...
#' @export
CoordCartPolar2 <- ggproto("CoordCartPolar2", CoordCartesian,
  ...
  transform = function(self, data, panel_params) {
    print("transform")
    rescale_x <- function(data) xy_rescale(data, panel_params$x.range)
    rescale_y <- function(data) xy_rescale(data, panel_params$y.range)
    print(data)
    data <- ggplot2::transform_position(data, rescale_x, rescale_y)
    print(data)
    data
  },
  ...
  render_bg = function(self, panel_params, theme) {
    ...
    if (inherits(xminor_element, "element_line")) {
      ...
      # xminor <- r_rescale(self, xminor, panel_params$x.range)
    ...
    if (inherits(xmajor_element, "element_line")) {
      ...
      # xmajor <- r_rescale(self, xmajor, panel_params$x.range)
    ...
    if(!is.null(self$scale_wd)) {
      ...
      if (length(labels) > 0 & length(theta) > 0) {
        ...
        x0 <- 0.45 * sin(theta) + 0.5
        y0 <- 0.45 * cos(theta) + 0.5
  ...
  setup_panel_params = function(self, scale_x, scale_y, params) {
    ...
    train_cartpolar <- function(scale, limits, name, order) {
      ...
      print(res)
      res$major <- xy_rescale(res$major, c(0, 1))
      res$minor <- xy_rescale(res$minor, c(0, 1))
      res$arrange <- order
...
xy_rescale <- function(xy, range) {
  xy <- scales::squish_infinite(xy, range)
  scales::rescale(xy, c(0.10, 0.90), range)
}
