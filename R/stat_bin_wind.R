...
#' @param z z values to be summarised
...
#' 
stat_bin_wind <- function(ws, wd, z, groups = NULL, fun = "mean", fun.args = list(), nmin = 3, ws_max = Inf, bins = 100, 
  ...
  if (fun != "identity") { 
    ...
    data <-  
      ...
      dplyr::mutate(
        ...
        ws_class = cut(ws, breaks = seq(0, max(pmin(ws, ws_max, na.rm = TRUE)), ws_binwidth))
      ) %>% 
      na.omit() %>% 
    ...
    if ("u_class" %in% groups & "v_class" %in% groups) {
      ...
      if (smooth) {
        data_smooth <- 
          ...
          dplyr::group_by(stat) %>%
          do({
            gam_surface(u = .$u, v = .$v, z = .$z, weights = pmin(4, .$ns) / 4,
